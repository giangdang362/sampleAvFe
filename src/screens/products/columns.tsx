...
import { faPlus } from "@/lib/fas/pro-light-svg-icons/faPlus";
        ...
        >
          {original?.row?.images?.length ? (
            <Box
              ...
              alt={`${original.row.series}-${original.row.model}-image`}
    ...
    },
    {
      ...
      renderCell: (original: GridRenderCellParams<API.ProductItem>) => {
        return (
          ...
          >
            <Tooltip title={t("add_to")} arrow>
              <IconButton
                aria-label="icon button"
                onClick={(event) => {
                  event.stopPropagation();
                  handleClickOpen();
                  setCurId(original?.row?.id ?? "");
                }}
              >
